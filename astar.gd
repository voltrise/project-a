...
# Helper class untuk merepresentasikan node pada algoritma A*
class PathNode:
	var position: Vector2i
	var parent: PathNode = null
	var g: float = 0.0 # Biaya kumulatif dari start ke node ini
	var h: float = 0.0 # Heuristik perkiraan jarak ke target (Manhattan)
	var f: float = 0.0 # Total evaluasi (f = g + h)
...
# Jarak Manhattan heuristik antara dua koordinat tile
static func _get_heuristic(a: Vector2i, b: Vector2i) -> float:
	return abs(a.x - b.x) + abs(a.y - b.y)

# Mengambil node dengan nilai F terendah dari open list
...
# Algoritma utama A* dengan perekaman visualisasi langkah debug & bobot path
